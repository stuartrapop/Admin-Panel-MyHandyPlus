...
// Custom cluster icon creator - makes clusters much more visible
...
const createClusterCustomIcon = (cluster: any) => {
	...
	// Size clusters based on number of markers
	if (count < 10) {
		size = 'small';
		clusterSize = 40;
	} else if (count < 100) {
	...
	return L.divIcon({
		...
		iconSize: L.point(clusterSize, clusterSize, true),
	});
};
...
const UserPopup: React.FC<UserPopupProps> = ({ userId }) => {
		...
		const fetchUserDetails = async () => {
			try {
				...
				if (data && data.length > 0) {
					...
					// Fetch signed URL for avatar if storage_key exists
					if (data[0].avatar_url) {
						setPhotoLoading(true);
						try {
							const { data: signedData, error: signedError } = await supabase.storage
								.from('images')
								.createSignedUrl(data[0].avatar_url, 3600);
	...
	if (loading) {
		return (
			<div style={{ padding: '16px', textAlign: 'center' }}>
				...
				<div style={{ marginTop: '8px', color: '#666' }}>Loading user details...</div>
			</div>
		);
	}
	...
	// Render avatar with loading/error/placeholder states
	const renderAvatar = () => {
		if (photoLoading) {
			return (
				<div
		...
		if (photoUrl) {
			...
			return (
				<img
					...
					onError={(e) => {
						// Hide the image on error and show error placeholder
			...
			<div style={{ borderTop: '1px solid #e0e0e0', paddingTop: '12px' }}>
				{details.city && details.country && (
					<div style={{ marginBottom: '8px', fontSize: '14px' }}>
						...
						<span style={{ color: '#333' }}>{details.city}, {details.country}</span>
					</div>
				)}
				{details.gender && (
					<div style={{ marginBottom: '8px', fontSize: '14px' }}>
						<strong style={{ color: '#555' }}>Gender:</strong>{' '}
						<span style={{ color: '#333' }}>{details.gender}</span>
					</div>
				)}
				<div style={{ marginBottom: '8px', fontSize: '14px' }}>
					...
					<span
						style={{
							color: details.account_status === 'active' ? '#2e7d32' : '#d32f2f',
							fontWeight: 500,
						}}
					>
